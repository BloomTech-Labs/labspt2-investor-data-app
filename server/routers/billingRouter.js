...
router.post ('/', (req, res, next) => {
  ...
  if (bills.accountType && bills.usersId) {
    billing
      .insert (bills)
      .then (bills => {
        res.status (201).json (bills);
    ...
    res.status (400).json ({
      message: 'missing full street address, full mailing address, credit card number, expires month & year, security code, monthly bill amount, account status, user_id and/or # of targets used..',
    });
  }
});
