...
class TestMacd extends React.Component{
    constructor(){
        super();
        this.state = {
            timeStamp: {},
            companies: ['MMFT'], // stock company symbols
            stocks1: [],
            stocks2: [],
        }
    }
    ...
    componentDidMount(){
        ...
        let promises = this.state.companies.map(company =>   // map that sends array of companies through axios to invoke external API
           // axios.get(`https://www.alphavantage.co/query?function=TIME_SERIES_DAILY&symbol=${company}&interval=5min&apikey=ZV7Y9QKGXRHCY0A4`));
            axios.get(` https://www.alphavantage.co/query?function=EMA&symbol=${company}&interval=weekly&time_period=12&series_type=open&apikey=ZV7Y9QKGXRHCY0A4`));
        ...
        axios
            ...
            .then(results => {
                results.forEach(result => {  // loops through keys to access targeted values of stock(s)
                    ...
                    let data = result.data['Technical Analysis: EMA'] //Accesses correct object within API
